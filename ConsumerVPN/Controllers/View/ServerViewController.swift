...
protocol ServerListViewDelegate {
...
class ServerViewController : BaseViewController {
    ...
    var delegate: ServerListViewDelegate?

	@IBOutlet weak fileprivate var searchView: ColorView!

	@IBOutlet weak var searchField: NSSearchField!

	fileprivate var presentedTableData : [City] = []
    ...
    fileprivate func configureViewProperties() {
        ...
        for tableColumn in tableView.tableColumns {
            ...
            let headerCell = CustomColorHeaderCell(textCell: tableColumn.headerCell.stringValue)
    ...
    @IBAction func backButtonClicked(_ sender: BackButton) {
    ...
    fileprivate func scrollTableToVPNConfiguration() {
        ...
        if let index = presentedTableData.firstIndex(where: { $0 == vpnConfiguration?.city }) {
            let indexSet: IndexSet = [index+1]
